...
#pragma clang diagnostic pop
    ...
    [self performSelector:@selector(timeoutCheck:) withObject:nil  afterDelay:1.0];
    
//    NSLog(@"start recording filename:%@ delegate:%@  %zd",filename,self.delegate,[self.delegate respondsToSelector:@selector(recordingProgress:)]);
}
...
- (void)timeoutCheck:(id)sender {
    if (dateStartRecording){
        ...
        if (self.delegate && time > self.maxRecordingTime){
